...
```
Before using any software, it is clear that some values make no sense. For example, BOS has a player with a negative RBI. check_that is very useful as it allows me to create rules and check them. Rules such as valid answers, or logical answers. This is first done for the league as a whole.  
...
Here validation is good, but I need to make one that works, becuase it can let me check if there are any controntations, fails, warnings, or erros
```{r}
