...
The NCAA suggests that August through November are the best period, followed by December to March, and finally April to July.
...
One easy way to predict how likely it is that a player scored a hit would be by looking at the mean. since the mean is 0.55 in a binary disctibution, is means that any given player has a more than 50% chance of having scored a hit. This would be too easy, so we are doing to look at things a bit deeper.
...
What are the chances that someone scored a hit based on the hand that they bat?
I can use a crosstable for this and a chi squared to see the likelihood of having scored a hit within different populations. Based on this, while right-handed batters have a 50-50 changes of having scored a hit, left handed are more likely. The four ambidextrous players all scored a hit.
```{r}
CrossTable(BOSLAACleanSeasonHit, BOSLAACleanBats, digits=2, prop.r = TRUE, prop.c = TRUE,  prop.chisq = FALSE, 
           chisq = TRUE, fisher = TRUE)
```
Because the p value is above 0.05, handedness is not a significant determinant.
```{r}
...
```
Here I see that the most significant factors are whether somebody played a game, their salary, their height, and the number of hits.
```{r}
exp(coef(HitsLM))
...
Now I will do logistic regression
hits need to be a factor in this case, though. 
```{r}
...
Here, career legth, heights, and alary are the most significant factors.
...
The source for using this is lectures 6 and 7, also this:
http://www.sthda.com/english/articles/40-regression-analysis/166-predict-in-r-model-predictions-and-confidence-intervals/


# References  
The Art of Data Science, Peng & Matsui
Skewness:  https://help.gooddata.com/doc/en/reporting-and-dashboards/maql-analytical-query-language/maql-expression-reference/aggregation-functions/statistical-functions/predictive-statistical-use-cases/normality-testing-skewness-and-kurtosis
NCAA: http://www.ncaa.org/about/resources/research/birthday-effect-college-athletics
Athletic Performance and Birth Month: Is the Relative Age Effect More than just Selection Bias?, DOI: 10.1055/s-0034-1368725
The Book of Why, Pearl
The R Book, Crawley
Practical Data Science with R, Manning
Problem Solving, a Statistician's Guide, Chatfield
Data Wrangling with R, Engel
...
Merging: https://www.statmethods.net/management/merging.html
GGPLOT2: http://www.sthda.com/english/wiki/ggplot2-line-plot-quick-start-guide-r-software-and-data-visualization#create-line-plots-with-points
Data Quality Checking: http://www.sthda.com/english/wiki/ggplot2-line-plot-quick-start-guide-r-software-and-data-visualization#create-line-plots-with-points
Data Cleaning: https://www.loom.com/share/5fdbaf30334047549ee6893a40987cec
Linear Regression: https://www.studystats.com/2020/11/glm.html
Introduction to R: http://people.brunel.ac.uk/~csstssd/itr/index.html
